...
def check_collicions(screen: pygame.surface.Surface,snake_body: pygame.sprite.Group, apples : pygame.sprite.Group, audio: Audio, scoreboard: Scoreboard ) -> bool:
    """
...
def screen_refresh(screen: pygame.surface.Surface, clock: pygame.time.Clock,
    ...
    for snake_block in reversed(snake_body.sprites()):
        # se anima el movimiento de la serpiente
        snake_body.sprites()[0].anime_snake()
        snake_block.blit(screen)

    # se anima el movimiento de la manzana
    apples.sprites()[0].anime_apple()
